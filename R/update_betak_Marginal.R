...
update_betak_Marginal <- function(C, sigma2, Phi, Omega, Psi,  prior.mu, prior.Sigma, Y, Vf, Vr, subjectID) {
  ...
  for (k in 1:K) {
    ...
    llikContr <- sapply(unique(subjectIDk), function(x) {
      ...
      # Marginal variance for each subject based on class k
      Rik <- (Vri %*% Phik %*% t(Vri)) + diag(sigma2k, nrow = length(Yi), ncol = length(Yi)) + matrix(rep(Omegak, length(Yi) * length(Yi)), nrow = length(Yi), ncol = length(Yi)) + matrix(rep(Psik, length(Yi) * length(Yi)), nrow = length(Yi), ncol = length(Yi))
