...
update_alpha_BSpline_MNP <- function(Z, delta, deltaStratum, u, nu, W, B, prior.Sigma, clusterIDSub, stratumIDSub) {
  ...
  for (k in 1:(K - 1)) {
    ...
    if (!is.null(nu) & is.null(u)) {
      mu[ , k] <-  Z[ , k] - W %*% delta[ , k] - nu[clusterIDSubf, k] - deltaStratum[factor(stratumIDSub), k]
    }

    # 2. Unstructured heterogeneity at the cluster level only
    if (is.null(nu) & !is.null(u)) {
      mu[ , k] <-  Z[ , k] - W %*% delta[ , k] - u[factor(clusterIDSub), k]  - deltaStratum[factor(stratumIDSub), k]
    }

    # 3. Spatially structured and unstructured heterogeneity at the cluster level only
    if (!is.null(nu) & !is.null(u)) {
      mu[ , k] <-  Z[ , k] - W %*% delta[ , k] - u[factor(clusterIDSub), k]  - deltaStratum[factor(stratumIDSub), k] - nu[clusterIDSubf, k]
    }
